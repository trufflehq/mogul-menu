...
import { context, useObservables, Stream, Component, Legacy } from '@spore/platform'
...
export default function BrowserExtensionWaysToEarn ({ enqueueSnackBar }) {
  ...
  if (!enqueueSnackBar) console.warn('[browser-extension-ways-to-earn] enqueueSnackBar not defined')
  ...
  const logos = [
    {
      ...
      imgUrl: 'https://cdn.bio/ugc/file/74d63ae0-3134-11ec-b844-c04b4cdda93a/39d5b910-5f6b-11ec-84a1-2e2f04e55ed6.svg',
      color: '#586AEA',
      hasConnection: false,
      data: {
        title: 'Discord'
    ...
    },
    {
      ...
      imgUrl: 'https://cdn.bio/ugc/file/74d63ae0-3134-11ec-b844-c04b4cdda93a/39d59200-5f6b-11ec-932f-1a7d09fc7d5c.svg',
      color: '#5D92F7',
      hasConnection: false,
      data: {
        title: 'Twitter'
  ...
  const { oAuthLinks, connections, me } = useObservables(() => ({
    oAuthLinks: model.connection.getOAuthUrlsBySourceTypes(['discord', 'twitch', 'twitter']),
    connections: connectionsObs,
    me: model.user.getMe()
  }))

  const isConnectionsChanged = !Legacy._.isEqual(connections, lastUpdatedConnectionsRef.current)

  useEffect(() => {
    if (isConnectionsChanged && lastUpdatedConnectionsRef?.current) {
      const updatedConnections = Legacy._.differenceBy(connections, lastUpdatedConnectionsRef?.current, 'id')
      ...
      if (!Legacy._.isEmpty(updatedConnections)) {
        ...
        enqueueSnackBar(() => <AccountConnectedSnackBar connectionImgSrc={imgSrc} connectionTitle={connectionTitle} />)
      }
    }
  ...
  const hasConnection = (selectedSource) => Legacy._.find(connections, { sourceType: selectedSource })
  ...
  const displayedConnections = connections?.filter(
    connection => logos.map(logo => logo.sourceType).includes(connection.sourceType)
  )
    ...
    <div className="c-browser-extension-earn-xp">
      <div className="connections">
        { logos.filter((logo) => !hasConnection(logo.sourceType))
          ...
          .map((logo, idx) => (
          <a className="connection" target={'_blank'} key={ idx } href={oAuthUrlMap ? oAuthUrlMap[logo?.sourceType] : ''} onClick={() => openOAuth(logo?.sourceType)} rel="noreferrer">
            <img src={ logo?.imgUrl ?? model.image.getSrcByImageObj(logo?.imgFileObj) } />
          </a>
          ))}
      </div>
      ...
      <div className="connected-connections">
        { logos.filter((logo) => hasConnection(logo.sourceType))
          .map((logo, idx) => (
          <div className="connection" style={`border: 1px solid ${logo.color};`} key={ idx }>
            <div className="left">
              <img src={ logo?.imgUrl ?? model.image.getSrcByImageObj(logo?.imgFileObj) } />
