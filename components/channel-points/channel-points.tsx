...
// HACK: for faze
const isFaze = typeof window !== 'undefined' && window.location?.hostname === 'faze1.live'
...
export default function $channelPoints (props) {
  ...
  if (!onFinishedCountdown) console.log('[channel-points]: onFinishedCountdown not defined')
  ...
  const {
    ...
    const claimChannelPointEconomyActionObs = model.economyAction.getByEconomyTriggerId(CHANNEL_POINTS_CLAIM_TRIGGER_ID)
    const claimXpEconomyActionObs = model.economyAction.getByEconomyTriggerId(XP_CLAIM_TRIGGER_ID)
    ...
    if (isFaze) {
      ...
      fazeWatchTimeObs = model.orgUserCounter.getMeByCounterTypeId(fazeWatchTimeId)
        .pipe(
          Stream.op.take(1),
          Stream.op.map((orgUserCounter) => {
            ...
            window._fStartTime = new Date(Date.now() - 1000 * secondsWatched).getTime()
    ...
    cookie.set(INITIAL_TIME_MS_COOKIE, initialTimeMs, { ttlMs: ALLOWED_TIME_AWAY_MS })
    // set initial
    cookie.set(LAST_CLAIM_TIME_MS_COOKIE, decrementTimeMs, { ttlMs: ALLOWED_TIME_AWAY_MS })

    const claimXpEconomyActionAndClaimChannelPointActionObs = Stream.Obs.combineLatest(
      claimXpEconomyActionObs, claimChannelPointEconomyActionObs
    )
    ...
    return {
      ...
      claimTimerCountdownSecondsObs: claimXpEconomyActionAndClaimChannelPointActionObs.pipe(
        Stream.op.map(([claimXpEconomyAction, claimChannelPointEconomyAction]) => {
  ...
  const incrementTimer = async () => {
    ...
    const secondsSinceInitialLoad = secondsSinceByMilliseconds(updatedTime, initialTimeRef.current)
    ...
    if (shouldUpdateWatchTime) {
      // refresh this cookie for another ALLOWED_TIME_AWAY_MS
      cookie.set(INITIAL_TIME_MS_COOKIE, initialTimeRef.current, { ttlMs: ALLOWED_TIME_AWAY_MS })
      // refresh last claim time cookie
      cookie.set(LAST_CLAIM_TIME_MS_COOKIE, cookie.get(LAST_CLAIM_TIME_MS_COOKIE), { ttlMs: ALLOWED_TIME_AWAY_MS })
  ...
  const onClaimHandler = async () => {
    cookie.set(LAST_CLAIM_TIME_MS_COOKIE, Date.now(), { ttlMs: ALLOWED_TIME_AWAY_MS })
    ...
    if (source?.sourceType) {
      const economyTransactions = await model.watchTime.claim(source.sourceType)

      const channelPointsClaimed = Legacy._.find(economyTransactions, { amountId: claimChannelPointEconomyAction?.amountId })?.amountValue || claimChannelPointEconomyActionAmount
      const xpClaimed = Legacy._.find(economyTransactions, { amountId: claimXpEconomyAction?.amountId })?.amountValue || claimXpEconomyActionAmount
  ...
  const darkChannelPointsSrc = model.image.getSrcByImageObj(darkChannelPointsImageObj) ?? 'https://cdn.bio/assets/images/features/chrome_extension/channel-points-default-dark.svg'
  ...
  return (
    <div className="z-channel-points" title={fullTitle}>
      { <div
        className={`claim ${classKebab({ hasText, isVisible: isClaimButtonVisible })}`}
        style={{
