...
import { formatCountdown, formatNumber, abbreviateNumber } from 'https://tfl.dev/@truffle/utils@0.0.1/format/format.js'
...
const TWITCH_ICON_POSITIONS = [
  {
    ...
    layoutConfigSteps: [
      ...
      { action: 'setStyle', value: { height: '55px', 'align-items': 'center', position: 'relative' } },
      { action: 'querySelector', value: '.chat-input__buttons-container > div' },
      { action: 'setStyle', value: { height: '55px', 'align-items': 'center', 'margin-left': '45px', position: 'relative' } },
...
export default function $browserExtensionMenu (props) {
  ...
  const {
    isSignupVisibleSubject, isOnboardTooltipVisibleSubject, credentialsSubject, connectionObs,
    activePollConnectionObs, isOpenSubject, activeTabIndexSubject, tabBadgeStatesSubject,
    channelPointsOrgUserCounterObs, channelPointsOrgUserCounterTypeObs,
    seasonPassObs, extensionIconBadgeStateObs, transactionsObs, snackBarQueueSubject,
    timeWatchedSecondsSubject, secondsRemainingSubject, isLiveObs, refreshSubject,
    isClaimableSubject, pageStackSubject, sourceTypeConnectionObs, extensionIconPositionSubject,
    hasViewedOnboardTooltipSubject, sourceObs
  } = useMemo(() => {
    try {
      ...
      const channelPointsOrgUserCounterObs = channelPointsOrgUserCounterTypeObs.pipe(
        op.switchMap((orgUserCounterType) => {
          if (orgUserCounterType) {
            const { invalidateFn, obs } = getModel().orgUserCounter.getMeByCounterTypeId(
              orgUserCounterType.id, { shouldReturnInvalidateFn: true }
            )
      ...
      const { invalidateFn: invalidateSeasonPassFn, obs: seasonPassObs } = getModel().seasonPass.getCurrent({ shouldReturnInvalidateFn: true })
      ...
      const transactionsObs = getModel().economyTransaction.getAll({
        ...
        op.map((economyTransactionConnection) => {
          return economyTransactionConnection?.nodes ? economyTransactionConnection.nodes : []
        })
      )
      const activePollConnectionObs = getModel().poll.getAllSmall({ isSubjected: true })
      ...
      const refreshAndSourceObs = Obs.combineLatest(refreshSubject.obs, sourceObs)
      ...
      const isLiveObs = refreshAndSourceObs.pipe(
        ...
        op.switchMap(([refresh, source]) => {
          return source
            ...
            ? getModel().channel.getBySource({ sourceType: source.sourceType, ignoreCache: true })
            : Obs.of(null)
        }),
      ...
      const sourceTypeConnectionObs = credentialsSubject.obs.pipe(
        op.switchMap((credentials) => {
          return getModel().connection.getConnectionByMeAndSource(credentials?.sourceType)
        })
      )
      ...
      return {
        ...
        hasViewedOnboardTooltipSubject: createSubject(hasViewedOnboardTooltipObs),
        sourceObs
      }
    } catch (err) {
      console.log('err', err)
  ...
  const {
    me, isOpen, credentials, hasNotification, activePollConnection,
    activeTabIndex, tabBadgeStates, seasonPass,
    channelPoints, isLive, isClaimable, snackBarQueue, pageStack, org,
    sourceTypeConnection, extensionIconPosition, hasViewedOnboardTooltip,
    extensionInfo, source, orgUserWithKv
  } = useObservables(() => ({
  ...
  const shouldShowOnboardTooltip = hasViewedOnboardTooltip === false && extensionIconPosition === 'stream-top-right'
  ...
  }, [isOpen, isClaimable, snackBarQueue, extensionIconPosition, extensionInfo, shouldShowOnboardTooltip])
  ...
  const onViewCollection = () => {
    ...
    const collectibleTabIndex = visibleTabs.findIndex(tab => tab.slug === 'collection')
  ...
  const transferJwt = async () => {
    await getModel().mogulTv.signInWithMogulTvJwt(credentials.token, { isTransfer: true })
  }
  ...
  const isNotFazeOrIsFazeEligible = !isFaze || (
    orgUserWithKv?.keyValueConnection?.nodes?.find(({ key }) =>
      key === 'isEligibleMerchGiveaway')?.value === 'yes' &&
  ...
  const shouldShowTwitchBanner = !isFaze && getModel().user.isMember(me) && credentials?.sourceType === 'twitch' && !hasConnectedAccount

  const className = `c-browser-extension-menu position-${extensionIconPosition} ${classKebab({ isOpen, hasNotification, isClaimable })}`
  ...
  return (
    <div className={className}>
        ...
        <div className="inner">
          ...
          }
          {
            !isPageStackEmpty
              ...
              : <div className="body">{<$activeTabEl {...{
                handleSignup,
                org,
                extensionInfo,
                activePoll,
      ...
      <SnackBarContainer snackBarQueueSubject={snackBarQueueSubject} visibilityDuration={SNACKBAR_ANIMATION_DURATION_MS} />
...
function getMenuState ({ isOpen, isClaimable, snackBarQueue, shouldShowOnboardTooltip }) {
  if (isOpen) {
    return 'open'
...
function NewExtensionUserTooltip ({ hasViewedOnboardTooltipSubject, $$extensionIconRef }) {
  const onClose = () => {
    jumper.call('storage.set', { key: getStorageKey(STORAGE_TOOLTIP_PREFIX), value: true })
        ...
        $content: (
          <div className='c-extension-user-tooltip use-css-vars-creator'>
            ...
            <div className='button'>
              <Button text='Got it' size='medium' style='primary-outline' onclick={onClose} />
            </div>
          </div>
        )
      }}
...
function getBaseStyles ({ extensionIconPosition, extensionInfo }) {
  const twitchPageIdentifiers = getTwitchPageIdentifier(extensionInfo?.pageInfo)
...
function getIframeStyles ({ state = 'open', extensionIconPosition, extensionInfo }) {
...
function setMenuStyles ({ state, jumper, extensionIconPosition, extensionInfo }) {
  const style = getIframeStyles({ state, extensionIconPosition, extensionInfo })
...
function ActionBanner ({ onClick, message, buttonText, classNamePostfix }) {
  return <div className={`action-banner action-banner-style-${classNamePostfix}`}>
    <div className='info'>
      { message }
    </div>
...
function SettingsPage ({ extensionIconPositionSubject, extensionInfo, onBack }) {
  ...
  const hasSporeUserSettings = extensionInfo?.version && extensionInfo.version.localeCompare(
    minVersionForSporeUserSettings, undefined, { numeric: true, sensitivity: 'base' }
  ) !== -1
  ...
  const save = async () => {
    await getModel().user.requestLoginIfGuest(me, overlay, { props: { source: 'extension-profile' } })
...
function PositionChooser ({ extensionInfo, extensionIconPositionSubject }) {
  ...
  const hasLayoutConfigSteps = extensionInfo?.version && extensionInfo.version.localeCompare(
    minVersionForLayoutConfigSteps, undefined, { numeric: true, sensitivity: 'base' }
  ) !== -1 && allowsThirdPartyCookies

  const positions = getExtensionPositions({ extensionInfo })

  const move = (position) => {
    const layoutConfigSteps = positions.find(({ positionSlug }) => positionSlug === position).layoutConfigSteps
    jumper.call('storage.set', { key: getStorageKey(STORAGE_POSITION_PREFIX), value: position })
    jumper.call('layout.setDefaultLayoutConfigSteps', { layoutConfigSteps })
    extensionIconPositionSubject.next(position)
  }

  return <div className="c-position-chooser">
    ...
    { hasLayoutConfigSteps && <>
      { positions.map((position) => {
        const isSelected = (extensionIconPosition || 'stream-top-right') === position.positionSlug
        return <div
...
function PredictionPage ({
  ...
  const channelPointsSrc = channelPointsImageObj ? getModel().image.getSrcByImageObj(channelPointsImageObj) : 'https://cdn.bio/assets/images/features/browser_extension/channel-points-default.svg'
  let pageContent
  if (!activePoll) {
    pageContent = (
      <div className='c-prediction-page_no-active-predictions'>
        <div>
          <Icon icon={CRYSTAL_BALL_ICON} color={cssVars.$bgBaseText60} size='40px' viewBox={CRYSTAL_BALL_ICON_VIEWBOX} />
        </div>
        <div className='text'>No active predictions</div>
      </div>
    )
  } else {
    pageContent = <Component slug="active-prediction"
      props={{
        isForm: true
      }}
    />
      ...
      props={{
        ...
        headerTopRight:
          <div className="c-predictions-page_channel-points">
              <div className="icon">
                <ImageByAspectRatio imageUrl={channelPointsSrc} aspectRatio={1} widthPx={16} height={16} />
              </div>
  ...
  return (
    <Component
      ...
      props={{
        ...
        popPage,
        $emptyState:
          (props) => <ExtensionMenuCollectibleEmptyState
            org={org}
...
function ChannelPointsShopTab (props) {
  const { channelPointsOrgUserCounterObs, channelPointsImageObj, onViewCollection, enqueueSnackBar } = props
  ...
  return <>
    <Component slug="channel-points-shop" props={{
      channelPointsOrgUserCounterObs,
      channelPointsImageObj,
      onViewCollection,
      onHowToEarnClick,
...
function SeasonPassTab (props) {
  ...
  const xpSrc = props?.xpImageObj ? getModel().image.getSrcByImageObj(props.xpImageObj) : 'https://cdn.bio/assets/images/features/browser_extension/xp.svg'

  const onHowToEarnClick = () => {
    overlay.open(() => (
      <XpActionsDialog xpSrc={xpSrc} />
    ))
  }
  return <div className="c-mogul-pass-tab">
    <SeasonPass
    ...
    <div className="description">Connect your accounts to start earning XP</div>
    <div className="learn-more" onClick={onHowToEarnClick}>How do I earn XP?</div>
    {org?.slug !== 'faze' && <Component slug="browser-extension-ways-to-earn" props={{ enqueueSnackBar }} />}
  </div>
}
...
function HomeTab ({
  ...
  const { activePowerupsObs } = useMemo(() => {
    const orgUserActivePowerupConnectionObs = getModel().orgUser.getMeActivePowerupsWithJsx()
  ...
  const handleOpenNotificationDialog = () => {
    overlay.open(() => (
      <Component slug="browser-extension-notification-dialog"
      props={{
        transactionsObs,
        channelPointsImageObj,
        seasonPassObs,
  ...
  return <div className="c-home-tab">
      ...
      <div className='user'>
        ...
        <div className="info">
          ...
          <div className="amounts">
            { hasChannelPoints && <div className="amount">
              <div className="icon">
                <ImageByAspectRatio imageUrl={channelPointsSrc} aspectRatio={1} widthPx={20} height={20} />
              </div>
            ...
            <div className="amount">
              <div className="icon">
              <ImageByAspectRatio imageUrl={xpSrc} aspectRatio={1} widthPx={24} height={24} />
              </div>
              <div className="amount">
    ...
    <div className='tile-grid'>
      ...
      { ['faze', 'dev'].includes(org?.slug) && <>{/* HACK/hardcode for faze1 */}
...
function FazeSponsors () {
  const sponsors = [
    {
      imageSrc: 'https://cdn.bio/assets/images/creators/faze/moonpay_sponsor.png',
      url: 'https://www.moonpay.com'
    }
  ]
  return <div className="c-faze-sponsors">
    {sponsors.map(({ imageSrc, url }) => <a
      key={url}
      href={url}
      target="_blank"
      rel="noreferrer"
...
function FazeGiveaway ({ handleSignup }) {
  ...
  const isAlmostEligibleCryptoGiveawayKv = meOrgUserWithKv?.keyValueConnection?.nodes
  ...
  const hasFilledOutEligibility = isEligibleCryptoGiveawayKv || isEligibleMerchGiveawayKv
  ...
  const isAlmostEligibleCryptoGiveaway = isAlmostEligibleCryptoGiveawayKv?.value === 'yes'
  ...
  const configs = {
    ...
    almostEligibleCryptoGiveaway: {
      ...
      onclick: async () => {
        overlay.open(() => <Component slug="faze-eligibility-dialog" props={{ onlyNeedsEthWallet: true }} />)
      },
  ...
  const status = !hasFilledOutEligibility
    ...
    : isEligibleCryptoGiveaway && isEligibleMerchGiveaway
      ? 'eligibleBoth'
      : !isEligibleCryptoGiveaway && isEligibleMerchGiveaway && isAlmostEligibleCryptoGiveaway
          ? 'almostEligibleCryptoGiveaway'
  ...
  return <div className="c-faze-giveaway">
    <div className="header-image">
    <ImageByAspectRatio imageUrl={'https://cdn.bio/assets/images/creators/faze/giveaway_header.png'} aspectRatio={568 / 36} isStretch={true} />
    </div>
    <div className="text">
      {config.text}
    </div>
    {config.onclick && <Button text={config.buttonText} size='large' style='bg' onclick={config.onClick} />}
  </div>
}
...
function LeaderboardTile () {
  const { leaderboardCountersObs } = useMemo(() => {
    ...
    const leaderboardCountersObs = seasonPassObs.pipe(
      op.switchMap((seasonPass) =>
        seasonPass ? getModel().orgUserCounter.getAllByCounterTypeId(seasonPass.orgUserCounterTypeId, { limit: LEADERBOARD_LIMIT }) : Obs.of(null)
      )
    )
  ...
  let userRanks =
    _.map(leaderboardCounters?.nodes, (orgUserCounter, i) => {
      return {
        ...orgUserCounter,
      ...
      op.switchMap(
        (orgConfig) =>
          orgConfig?.data?.kingOfTheHill?.userId
            ? getModel().orgUser.getByUserIdWithPowerups(orgConfig?.data?.kingOfTheHill?.userId)
            : Obs.of(null)
      )
    )
    return {
...
function PredictionTile ({ pushPage, channelPointsImageObj, channelPointsOrgUserCounterObs, popPage }) {
  const { activePollConnectionObs, pollMsLeftSubject, isPredictionExpiredObs } = useMemo(() => {
...
function Advert (props) {
  const {
    className, imageSrc, hashtag, tagline, buttonHref, buttonOnClick, buttonText, buttonBgColor, buttonTextColor
  } = props

  return (
    <div className={`c-advert ${className}`}>
      <div className='image'><img src={imageSrc} alt="Ad" /></div>
      <div className='content'>
        <div className='text'>
        ...
        <div className='actions'>
          <Button
            ...
            href={buttonHref} onclick={buttonOnClick}
          />
        </div>
      </div>
    </div>
...
function IsLiveInfo (props) {
  const { secondsRemainingSubject, timeWatchedSecondsSubject, highlightButtonBg, creatorName, hasChannelPoints, hasBattlePass } = props
  ...
  <div className='info'>
    ...
    <div className='grid'>
      {(hasChannelPoints || hasBattlePass) && <Timer timerSeconds={timeWatchedSeconds} message={'Time watched'} />}
      {(hasChannelPoints || hasBattlePass) && <Timer timerSeconds={secondsRemaining} message={'Time until reward'} />}
    </div>
  </div>
</div>
}
...
function Timer ({ timerSeconds, message }) {
  return <div className='timer'>
    { (<>
    <div className='time'>
      {timerSeconds !== undefined ? formatCountdown(timerSeconds, { shouldAlwaysShowHours: false }) : ''}
    </div>
    <div className='title'>
      {message}
    </div>
...
function ChannelPointsClaimSnackBar ({ channelPointsClaimed = 20, totalChannelPoints = 0, channelPointsImageObj, darkChannelPointsImageObj }) {
  // const channelPointsSrc = channelPointsImageObj ? getModel().image.getSrcByImageObj(channelPointsImageObj) : 'https://cdn.bio/assets/images/features/browser_extension/channel-points.svg'
  const darkChannelPointsSrc = channelPointsImageObj ? getModel().image.getSrcByImageObj(darkChannelPointsImageObj) : 'https://cdn.bio/assets/images/features/browser_extension/channel-points-default-dark.svg'

  return (
    <Component slug="snack-bar"
      props={{
        message: `${channelPointsClaimed} channel points added!`,
        value:
          <>
            <div>{abbreviateNumber(parseInt(totalChannelPoints) + parseInt(channelPointsClaimed), 1)}</div>
            <ImageByAspectRatio imageUrl={darkChannelPointsSrc} aspectRatio={1} width={16} height={16} />
          </>
      }}
    />
  )
...
function XpClaimSnackBar ({ xpClaimed = 1, totalXp = 0, xpImageObj, darkXpImageObj }) {
  // const xpSrc = xpImageObj ? getModel().image.getSrcByImageObj(xpImageObj) : 'https://cdn.bio/assets/images/features/browser_extension/xp.svg'
  const darkXpSrc = xpImageObj ? getModel().image.getSrcByImageObj(darkXpImageObj) : 'https://cdn.bio/assets/images/features/browser_extension/xp-dark.svg'
  ...
  return (
    <Component slug="snack-bar"
      props={{
        ...
        value: <>
        <div>{ abbreviateNumber(parseInt(totalXp) + parseInt(xpClaimed), 1)} </div>
        <ImageByAspectRatio imageUrl={darkXpSrc} aspectRatio={1} width={20} height={20} />
        </>
      }}
    />
  )
...
function ActivePredictionSnackBar ({ onLinkClick }) {
  return (
    ...
    props={{
      ...
      style: 'flat',
      value: <a className='c-new-prediction' onClick={onLinkClick}> Participate
      </a>
    }}
  />
  )
...
function ExtensionMenuCollectibleEmptyState (props) {
  const {
    org, groupedCollectibles, channelPointsImageObj, xpImageObj,
    visibleTabs, activeTabIndexSubject, hasChannelPoints, hasBattlePass
  } = props
  ...
  const stackEmotes = isFaze
    ? _.map(_.take(tickets?.collectibles, 2), (ticket) => getModel().image.getSrcByImageObj(ticket?.fileRel?.fileObj))
    : _.map(_.take(emotes?.collectibles, 2), (emote) => getModel().image.getSrcByImageObj(emote?.fileRel?.fileObj))

  const onViewBattlePass = () => {
    // navigate to the battle pass tab when the user
    const collectibleTabIndex = visibleTabs.findIndex(tab => tab.slug === 'battle-pass')
    activeTabIndexSubject.next(collectibleTabIndex)
  }

  const onViewShop = () => {
    // navigate to the shop tab when the user
    const collectibleTabIndex = visibleTabs.findIndex(tab => tab.slug === 'shop')
    activeTabIndexSubject.next(collectibleTabIndex)
  }
  ...
  return <div className='c-collectible-empty'>
    <div className='wrapper'>
      <div className='preview'>
        <div className='stack'>

          <div className='block front'>
            <ImageByAspectRatio imageUrl={stackEmotes?.[0]} aspectRatio={1} width={40} height={40} />
          </div>
          <div className='block back'>
            <ImageByAspectRatio imageUrl={stackEmotes?.[1]} aspectRatio={1} width={40} height={40} />
          </div>
        </div>
        <div className='message'>
     ...
     {(hasBattlePass || hasChannelPoints) && <div className='body'>
        <div className='title'>
          Start Earning
        </div>
       {
        hasBattlePass &&
       <WayToEarnBlock
          description={`Earn XP to unlock rewards in the ${getSeasonPassName(org)}`}
        ...
        {hasChannelPoints && <WayToEarnBlock
...
function WayToEarnBlock ({ description, iconSrc, linkText, onLinkClick, buttonText, onButtonClick }) {
  return <div className='way-to-earn'>
    <div className='left'>
      <div className='icon'>
        <ImageByAspectRatio imageUrl={iconSrc} aspectRatio={1} width={24} height={24} />
      </div>
    </div>
    ...
    <div className='right'>
      ...
      <div className='button'>
        <Button text={buttonText} size='large' style='primary' onclick={onButtonClick} />
      </div>
...
function ChannelPointsActionsDialog ({ channelPointsSrc }) {
  const { channelPointsEarnActionsObs } = useMemo(() => {
    ...
    const economyActionsObs = getModel().economyAction.getByAmountType('orgUserCounterType')

    const channelPointsEarnActionsObs = Obs.combineLatest(channelPointsOrgUserCounterTypeObs, economyActionsObs).pipe(
      op.map(([orgUserCounterType, economyActions]) => {
        return _.filter(economyActions?.nodes, { amountId: orgUserCounterType?.id })
      }),
      ...
      op.map((channelPointsActions) => _.filter(channelPointsActions, (action) => action?.name !== 'Prediction refund')),
...
function LearnMoreButton () {
  ...
  return <div className='c-learn-more'>
    <div className='title'>
      Earn XP through linked accounts
    </div>
    <a href={ getHost()} target="_blank" className='button' rel="noreferrer">
      <Button text='Learn more' size='medium' style='primary-outline' icon='openExternal' iconLocation='right' />
    </a>
  </div>
}
...
function XpActionsDialog ({ xpSrc }) {
  const { seasonPassWatchTimeActionsObs } = useMemo(() => {
    ...
    const seasonPassWatchTimeActionsObs = seasonPassObs.pipe(
      op.map((seasonPass) => {
        return _.filter(seasonPass?.economyActions, (action) => action?.economyTriggerId === XP_INCREMENT_TRIGGER_ID || action?.economyTriggerId === XP_CLAIM_TRIGGER_ID)
      })
    )
    return {
...
function EconomyActionDialog ({ title, economyActions, orgUserCounterTypeSrc, $bottom }) {
  return <div className='c-economy-action-dialog'>
          ...
          $content: <div className="content">
            {
              _.map(economyActions, (action) => {
                return <EconomyAction economyAction={action} orgUserCounterTypeSrc={orgUserCounterTypeSrc} />
              })
            }
            {
              <div className='bottom'>
...
function EconomyAction ({ economyAction, orgUserCounterTypeSrc }) {
  return <div className='c-economy-action'>
    <div className='name'>
      {economyAction?.name}
    </div>
    <div className='reward'>
      <div className='icon'>
        <ImageByAspectRatio imageUrl={orgUserCounterTypeSrc} aspectRatio={1} width={24} height={24} />
      </div>
      <div className='amount'>
        {economyAction?.amountValue > 0 ? economyAction?.amountValue : economyAction?.data?.amountDescription}
      </div>
    </div>
    <div className='description'>
